...
/* 更新结点的高度 */
static int AVLTreeNodeUptadeHeight(AVLTreeNode *node);
...
{
    /* 左子树高度 */
    int leftHeight = node->left == NULL ? 0 : node->right->height;
    /* 右子树高度 */
...
static int AVLTreeNodeIsBalanced(AVLTreeNode *node)
{
    ...
    int nodeF = AVLTreeNodeBalanceFactor(node);
    if (nodeF <= 1)
    {
        return true;
    }
